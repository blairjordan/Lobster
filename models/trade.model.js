...
Trade.findOffers = async (options) => {
  const { target_player_name } = options;
  return await db.any(`SELECT * FROM v_offers WHERE target = $1`, [target_player_name]);
};

Trade.addOffer = async (options) => {
  const { source_player_name, target_player_name } = options;
  return await db.one(`INSERT INTO offer(source_id, target_id, target_status) SELECT p1.player_id, p2.player_id, 'O' FROM player p1, player p2 
